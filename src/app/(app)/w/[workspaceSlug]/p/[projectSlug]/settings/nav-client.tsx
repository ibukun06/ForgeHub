...
export function ProjectNavMobile({ baseUrl }: { baseUrl: string }) {
  const pathname = usePathname();
  
  const activeSegment = PROJECT_SETTINGS_NAV.find(item => pathname.includes(`/${item.segment}`))?.segment || "general";
