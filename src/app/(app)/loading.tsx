          ...
          <div className="grid gap-3 sm:grid-cols-3 xl:min-w-[420px]">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="rounded-3xl border border-white/12 bg-white/6 px-4 py-3">
                ...
                <div className="mt-3 h-8 w-16 rounded-2xl bg-white/12" />
            ...
            {Array.from({ length: columnIndex === 2 ? 3 : 2 }).map((__, panelIndex) => (
              <div key={panelIndex} className="surface-panel p-5">
                <div className="mb-4 space-y-2">
                  <div className="h-6 w-36 rounded-2xl bg-border/70" />
                  <div className="h-4 w-2/3 rounded-full bg-border/55" />
                </div>
                <div className="grid gap-3">
                  {Array.from({ length: 3 }).map((___, itemIndex) => (
                    <div key={itemIndex} className="surface-panel-muted p-4">
                      ...
                      <div className="mt-3 h-3 w-full rounded-full bg-border/40" />
                      <div className="mt-2 h-3 w-4/5 rounded-full bg-border/35" />
                    </div>
                  ))}
                </div>
              </div>
