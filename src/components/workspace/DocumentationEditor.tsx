...
import { Check, ChevronRight, FileText, Save, Sparkles } from "lucide-react";
...
export function DocumentationEditor({ projectId, documents, initialDocumentId }: { projectId: string; documents: DocumentItem[]; initialDocumentId: string | null }) {
  ...
  function chooseDocument(id: string) { setDocumentId(id); }
  function chooseSection(id: string) { setSectionId(id); }
  ...
  return <div className="grid gap-8 xl:grid-cols-[240px_210px_minmax(0,1fr)]">
    <aside className="border-b border-border pb-5 xl:border-b-0 xl:border-r xl:pb-0 xl:pr-5"><p className="font-mono text-xs uppercase tracking-[0.12em] text-text-muted">Documents</p><nav className="mt-4 grid gap-1" aria-label="Project documents">{documents.map((document) => <button type="button" key={document.id} onClick={() => chooseDocument(document.id)} className={`flex min-h-11 items-center gap-2 rounded-md px-3 text-left text-sm ${document.id === activeDocument?.id ? "bg-primary/10 font-medium text-primary" : "text-text-muted hover:bg-surface hover:text-text-primary"}`}><FileText className="h-4 w-4 shrink-0" aria-hidden /><span className="truncate">{document.title}</span></button>)}</nav></aside>
    <aside className="border-b border-border pb-5 xl:border-b-0 xl:border-r xl:pb-0 xl:pr-5"><p className="font-mono text-xs uppercase tracking-[0.12em] text-text-muted">Sections</p><nav className="mt-4 grid gap-1" aria-label="Document sections">{activeDocument?.sections.length ? activeDocument.sections.map((section, index) => <button type="button" key={section.id} onClick={() => chooseSection(section.id)} className={`flex min-h-11 items-center justify-between gap-2 rounded-md px-3 text-left text-sm ${section.id === activeSection?.id ? "bg-surface font-medium text-text-primary" : "text-text-muted hover:bg-surface"}`}><span className="truncate">{section.prompt.split("?")[0]}</span>{section.status === "team_reviewed" && <Check className="h-4 w-4 shrink-0 text-success" aria-label="Reviewed" />}</button>) : <p className="text-sm leading-relaxed text-text-muted">This document is ready for its first section.</p>}</nav></aside>
