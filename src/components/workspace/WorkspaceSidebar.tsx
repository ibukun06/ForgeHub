  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-border bg-bg px-4 py-5 transition-transform duration-200 lg:static lg:z-auto lg:w-60 lg:translate-x-0 lg:rounded-lg lg:border ${open ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <nav aria-label="Workspace modules" className="grid gap-1">
        {ITEMS.map(({ slug, label, icon: Icon, enabled }) => enabled ? <Link key={slug} href={`/project/${projectId}/${slug}`} onClick={onClose} className="flex min-h-11 items-center gap-3 rounded-md bg-primary/10 px-3 text-sm font-medium text-primary"><Icon className="h-4 w-4" aria-hidden />{label}</Link> : <span key={slug} className="group flex min-h-11 cursor-not-allowed items-center justify-between rounded-md px-3 text-sm text-text-muted/70"><span className="flex items-center gap-3"><Icon className="h-4 w-4" aria-hidden />{label}</span><span className="hidden rounded-full border border-border px-2 py-0.5 font-mono text-[9px] uppercase tracking-wide group-hover:inline">Soon</span></span>)}
      </nav>
