...
export function MilestoneTimeline({ milestones }: { milestones: Milestone[] }) {
  return (
    <div className="relative mt-8 space-y-6 border-l border-border pl-6">
      {milestones.map((m) => (
        <div key={m.title} className="relative">
          ...
          <p className={`text-sm font-medium ${m.completed ? "text-text-primary" : "text-text-muted"}`}>{m.title}</p>
